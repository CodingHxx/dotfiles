...
if [ -n "$SELECTED" ]; then
    ...
    # ✅ SDDM mein bhi same wallpaper set karo
    sudo cp "$WALLPAPER_DIR/$SELECTED" /usr/share/sddm/themes/silent/backgrounds/current.jpg
    sudo sed -i 's|^background = .*|background = "current.jpg"|' /usr/share/sddm/themes/silent/configs/default.conf
fi
